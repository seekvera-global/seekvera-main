  ...
  try{
    ...
    mediaRecorder.onstop=async()=>{
      ...
      try{
        ...
        if(i){i.value=String(d.text).trim();i.placeholder='Message SEEKVERA AI…';if(i.value){voiceReplyDeadline=Date.now()+45000;setTimeout(()=>i.form?.requestSubmit?.(),45)}}
...
function start(targetId,button){
  ...
  r.onend=()=>{clearSpeechTimers();setMic(false);if(recognition===r)recognition=null;const q=i?.value?.trim();if(q&&!hadError){voiceReplyDeadline=Date.now()+45000;setTimeout(()=>i.form?.requestSubmit?.(),45)}else if(!q)voiceConversation=false};
...
window.addEventListener('seekvera:ai-response',e=>{const auto=voiceConversation||Date.now()<voiceReplyDeadline;if(auto)voiceReplyDeadline=0;speak(e.detail?.text,e.detail?.language,auto)});

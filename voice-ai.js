...
function updateSpeakerButtons(){document.querySelectorAll('#aiChatSpeaker,.sv-global-speaker').forEach(b=>{b.textContent=muted?'🔇':'🔊';b.setAttribute('aria-label',muted?'Turn voice replies on':'Turn voice replies off')})}
...
function unlockTTS(){if(!hasTTS())return;try{speechSynthesis.resume();const u=new SpeechSynthesisUtterance(' ');u.volume=0;u.rate=1;speechSynthesis.speak(u);setTimeout(()=>{try{if(!speechSynthesis.speaking)speechSynthesis.resume()}catch(_){}},80)}catch(_){}}
...
function speak(text,l,force=false){lastAnswer=typeof text==='string'?text:'';lastLocale=localeForText(lastAnswer,l);const spoken=cleanSpeech(lastAnswer);if(force){muted=false;localStorage.setItem('seekvera_voice_muted','0');updateSpeakerButtons()}if(muted||!spoken||!hasTTS()){if(force)voiceConversation=false;return}const token=++speakToken;const run=attempt=>{if(token!==speakToken)return;if(recognition){setTimeout(()=>run(attempt),180);return}try{speechSynthesis.cancel();speechSynthesis.resume();const u=new SpeechSynthesisUtterance(spoken);u.lang=lastLocale;const v=pickVoice(lastLocale);if(v)u.voice=v;u.rate=1.01;u.pitch=1.05;u.volume=1;let started=false,done=false;u.onstart=()=>{started=true};u.onend=()=>{done=true;if(token===speakToken)voiceConversation=false};u.onerror=()=>{if(done||token!==speakToken)return;if(!started&&attempt<TTS_RETRY_DELAYS.length-1)setTimeout(()=>run(attempt+1),TTS_RETRY_DELAYS[attempt+1]);else if(token===speakToken)voiceConversation=false};speechSynthesis.speak(u);setTimeout(()=>{if(done||started||token!==speakToken)return;if(!speechSynthesis.speaking&&attempt<TTS_RETRY_DELAYS.length-1)run(attempt+1)},TTS_RETRY_DELAYS[attempt]+650)}catch(_){if(attempt<TTS_RETRY_DELAYS.length-1)setTimeout(()=>run(attempt+1),TTS_RETRY_DELAYS[attempt+1]);else voiceConversation=false}};setTimeout(()=>run(0),force?TTS_RETRY_DELAYS[0]:60)}

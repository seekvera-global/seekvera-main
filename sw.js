...
self.addEventListener('fetch',e=>{const r=e.request;if(r.method!=='GET')return;const u=new URL(r.url);if(u.origin!==self.location.origin)return;const critical=/\/(?:i18n-ui|voice-ai|global-ui|superapp|navigation|locale-r15|locale-r14|locale-r14-categories|r20-final-guard|r20-extra-categories|r21-card-cleanup|r22-category-lock)\.js$/.test(u.pathname);if(r.mode==='navigate'||/\.html$/.test(u.pathname)||u.pathname.endsWith('/')||critical){e.respondWith((async()=>{try{const resp=await fetch(r,{cache:'no-store'});const out=await r21Html(resp);if(out?.ok){const copy=out.clone();caches.open(CACHE).then(c=>c.put(r,copy))}return out}catch{const x=await caches.match(r)||await caches.match('./index.html');return r21Html(x)}})());return}e.respondWith(fetch(r).then(resp=>{if(resp.ok){const copy=resp.clone();caches.open(CACHE).then(c=>c.put(r,copy))}return resp}).catch(()=>caches.match(r)))});

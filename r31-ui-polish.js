...
let switching=0,recognition=null,voiceTurn=false,voiceSeq=0,submitting=false;
...
function localizeCurrencies(){
 ...
 for(const code of [...codes].sort()){
  let o=[...el.options].find(x=>String(x.value||'').toUpperCase()===code);if(!o){o=document.createElement('option');o.value=code;el.appendChild(o)}o.textContent=currencyLabel(code);
 }
 const p=profiles[country()];if(p?.currency&&[...el.options].some(o=>o.value===p.currency))el.value=p.currency;
 el.setAttribute('aria-label',lang()==='ar'?'العملة':'Currency');
}
function repairCategoryIcons(){
 document.querySelectorAll('.r5-tile').forEach(a=>{const box=a.querySelector('.r5-thumb');if(!box)return;const key=(a.getAttribute('href')||'')+' '+(a.querySelector('b')?.textContent||'');let icon='';for(const [re,v] of ICONS){if(re.test(key)){icon=v;break}}if(icon&&box.textContent.trim()!==icon)box.textContent=icon;box.setAttribute('aria-hidden','true')});
}
function markDirection(){const l=lang();document.documentElement.lang=l;document.documentElement.dir=RTL.has(l)?'rtl':'ltr'}
...
function bindLocale(){document.addEventListener('change',e=>{if(e.target?.id==='country'||e.target?.id==='lang'||e.target?.id==='currency'){atomicLocale()}},true);new MutationObserver(ms=>{if(ms.some(m=>m.type==='childList')){repairCategoryIcons();localizeCurrencies()}}).observe(document.documentElement,{subtree:true,childList:true})}

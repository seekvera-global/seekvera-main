...
route=r'''if(u.pathname==='/api/ui-translate'){try{
 ...
 const raw='r33|'+language+'\u0000'+JSON.stringify(strings),dig=await crypto.subtle.digest('SHA-256',new TextEncoder().encode(raw)),hex=[...new Uint8Array(dig)].map(x=>x.toString(16).padStart(2,'0')).join('');
 const cache=await caches.open('seekvera-ui-translate-r33'),ck=new Request(new URL('/__sv_ui_translation_r33/'+hex,request.url).toString(),{method:'GET'}),hit=await cache.match(ck);
 ...
 if(!arr){
  ...
  if(tl&&tl!=='en'&&strings.every(x=>new TextEncoder().encode(x).length<=480)){
   try{
    ...
    if(vals.length===strings.length){arr=vals;model='mymemory-free-r33'}
   }catch(e){console.warn('UI MyMemory fallback',clean(e?.message||e,180))}
  }
 }
...
print('R33 universal translation backend patched with free translation-memory fallback')
...
# select options, and late-rendered marketplace status text).

p=Path('i18n-ui.js')
s=p.read_text(encoding='utf-8')
anchor="function markAttr(el,a,v){let m=RENDERED_ATTR.get(el);if(!m){m={};RENDERED_ATTR.set(el,m)}m[a]=String(v||'').trim()}\n"
insert=r'''function staticPackText(l,src){src=String(src||'').trim();if(!src||l==='en')return src;const t=PACKS.get(norm(l))?.translations||{};return String(t[src]||get(norm(l),src)||src).trim()}

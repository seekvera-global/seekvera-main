...
#    The confirmation is language-neutral, so it cannot reply in the
#    wrong language while the actual app state visibly changes.
# ------------------------------------------------------------------
...
# Pass browser language as a tiebreaker only; message language remains authoritative.
...
#    Both Workers AI and the zero-cost public backup self-detect the
#    latest message instead of trusting a possibly-wrong local hint.
# ------------------------------------------------------------------
...
# Browser/device language is only a tiebreaker for ambiguous Latin text when there is no detectable clue.
...
# ------------------------------------------------------------------
# 4) Voice chat is independent of country. Native SpeechRecognition
#    cannot auto-detect arbitrary languages, so use: explicit chat
#    language -> remembered conversation language -> DEVICE language.
#    Country/HTML language is intentionally NOT used as the default.
#    Whisper auto-ASR remains the fallback when native recognition is
#    unavailable.
# ------------------------------------------------------------------
...
# Remember the language actually used by the conversation for the next mic turn.
...
# Store response language from the AI; it reflects the latest user conversation language.
old="window.addEventListener('seekvera:ai-response',e=>{const d=e.detail||{};if(d.text)speak(d.text,d.language||'auto',voiceConversation||Date.now()<voiceReplyDeadline)});"
new="window.addEventListener('seekvera:ai-response',e=>{const d=e.detail||{};if(d.text){rememberChatVoiceLanguage(d.language,d.text);speak(d.text,d.language||'auto',voiceConversation||Date.now()<voiceReplyDeadline)}});"
if old in s:s=s.replace(old,new,1)
elif new not in s:raise SystemExit('R65 ai-response voice memory anchor missing')
# More alternatives help native recognition without changing the working one-tap flow.
s=s.replace('r.maxAlternatives=1;','r.maxAlternatives=3;',1)
p.write_text(s,encoding='utf-8')

# Release markers/cache.

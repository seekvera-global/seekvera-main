...
// anchors on every section. This preserves complete language + complete section coverage
// without the 3,000+ redundant Cartesian page-pack loads that exceeded Actions time.
...
await p.evaluate(()=>SEEKVERA_LOCALE_R15.setLanguage('en'));await pause(80);
...
for(let pi=0;pi<paths.length;pi++){
  ...
  await p.waitForFunction(()=>window.SEEKVERA_LOCALE_R15&&window.SEEKVERA_I18N_R32,{timeout:25000});
  await p.evaluate(()=>SEEKVERA_LOCALE_R15.setLanguage('en'));await pause(60);
...
console.log('R76_SECTION_LANGUAGE_PASS',JSON.stringify({routes:hrefs.length,paths:paths.length,languages:manifest.languages.length,combinations,coverage:'all-languages-and-all-sections-distributed'}));

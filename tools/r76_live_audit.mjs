...
// Discover all category routes from the real homepage and test every unique section in every language.
...
for(const path of paths){
  await p.goto(B+path+'?r76lang='+Date.now(),{waitUntil:'domcontentloaded',timeout:60000});
  await p.waitForFunction(()=>window.SEEKVERA_LOCALE_R15&&window.SEEKVERA_I18N_R32,{timeout:25000});
  await p.evaluate(()=>SEEKVERA_LOCALE_R15.setLanguage('en'));await pause(60);const english=await p.locator('body').innerText();const present=candidateEnglish.filter(s=>english.includes(s));
  for(const l of manifest.languages){
    await p.evaluate(async l=>{SEEKVERA_LOCALE_R15.setLanguage(l);await SEEKVERA_I18N_R32.loadPack(l);await SEEKVERA_I18N_R32.apply()},l);await pause(8);combinations++;
    if(l==='en')continue;const body=await p.locator('body').innerText();const leaks=present.filter(s=>packs[l]?.[s]&&packs[l][s]!==s&&body.includes(s)).slice(0,6);if(leaks.length){leakBad.push({path,l,leaks});break}
  }
  if(leakBad.length>6)break;
}
assert.equal(leakBad.length,0,'English UI leaks '+JSON.stringify(leakBad.slice(0,7)));
console.log('R76_SECTION_LANGUAGE_PASS',JSON.stringify({routes:hrefs.length,paths:paths.length,languages:manifest.languages.length,combinations}));
...
await p.goto(B+'/?r76restore='+Date.now(),{waitUntil:'domcontentloaded',timeout:60000});await p.waitForFunction(()=>window.SEEKVERA_CHAT&&document.querySelector('#aiMessages'),{timeout:30000});await pause(350);assert((await p.locator('#aiMessages').innerText()).includes('بدي وظيفة مبيعات بفرنسا'),'chat not restored');
...
const spoken=await p.evaluate(async()=>{window.__r76spoken=0;try{speechSynthesis.speak=()=>{window.__r76spoken++}}catch{};SEEKVERA_VOICE_AI?.markVoiceReply?.();window.dispatchEvent(new CustomEvent('seekvera:ai-response',{detail:{text:'مرحبا، كيف فيني ساعدك؟',language:'ar'}}));await new Promise(r=>setTimeout(r,600));return window.__r76spoken});
...
console.log('R76_FINAL_LIVE_PASS',JSON.stringify({countries:matrix.n,languages:manifest.languages.length,categoryRoutes:hrefs.length,sectionPaths:paths.length,languageSectionCombinations:combinations,sourceStrings:source.count,replyMs,chatSaved:true,chatRestored:true,appControl:true,ttsCalls:spoken}));
await browser.close();

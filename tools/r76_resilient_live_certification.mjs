...
const sectionLangs=['ar','fr','tr','hi','zh','de'];
async function checkSection(path,index){
 const p=await ctx.newPage();p.on('pageerror',e=>errors.push(path+': '+String(e)));
 try{
  const r=await p.goto(B+path+'?r76section='+Date.now()+'-'+index,{waitUntil:'domcontentloaded',timeout:60000});assert(r&&r.status()<400,path+' '+r?.status());
  await p.waitForFunction(()=>window.SEEKVERA_LOCALE_R15&&window.SEEKVERA_I18N_R32,{timeout:25000});await pause(180);
  for(const l of sectionLangs){await p.evaluate(async l=>{SEEKVERA_LOCALE_R15.setLanguage(l);await SEEKVERA_I18N_R32.loadPack(l);await SEEKVERA_I18N_R32.apply()},l);await pause(55);const txt=(await p.locator('body').innerText()).trim();assert(txt.length>20,path+' '+l)}
  console.log('R76_CATEGORY_PASS',index+1,paths.length,path);
 }finally{await p.close().catch(()=>{})}
}
for(let i=0;i<paths.length;i+=5){await Promise.all(paths.slice(i,i+5).map((path,j)=>checkSection(path,i+j)));console.log('R76_CATEGORY_BATCH_PASS',Math.min(i+5,paths.length),paths.length)}

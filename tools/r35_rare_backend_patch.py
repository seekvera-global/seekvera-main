...
if "u.pathname==='/api/r35-rare-translate'" in s:
    ...
    raise SystemExit(0)
route=r'''if(u.pathname==='/api/r35-rare-translate'){try{
 ...
 const b=await request.json(),code=clean(b.language,8).toLowerCase(),strings=Array.isArray(b.strings)?b.strings.slice(0,20).map(x=>clean(x,700)):[];
 ...
 const models=[FALLBACK,PRIMARY,LIGHT,FASTCHAT];let errors=[];
 ...
 }catch(e){return j(request,{ok:false,error:'Rare translation error',retryable:true,detail:clean(e?.message||e,160)},503)}}
